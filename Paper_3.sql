...
SELECT P.ProductName, COUNT(DISTINCT C.CustomerID) AS DifferentCustomerCount
FROM Products P JOIN Orders O ON P.ProductID = O.ProductID JOIN Customers C ON O.CustomerID = C.CustomerID
...
SELECT TOP 3 P.ProductName, SUM(O.TotalOrderAmount)
